...
const CORES = ["#294D80", "#E02F52", "#C4D2DE", "#4CAF50", "#FF9800", "#9C27B0", "#00BCD4"];
...
function MetricCard({ titulo, valor, subtitulo, tendencia, chip }) {
  const icone =
    tendencia > 0 ? (
      <TrendingUpIcon fontSize="small" sx={{ color: "error.main" }} />
    ) : tendencia < 0 ? (
        ...
        <>
          ...
          <Grid container spacing={2} sx={{ mb: 3 }}>
            <Grid item xs={6} sm={3}>
              <MetricCard
                titulo="Menor Preço"
                valor={fmt(m.menorPreco)}
                subtitulo="preço mais baixo hoje"
              />
            </Grid>
            <Grid item xs={6} sm={3}>
              <MetricCard
                ...
                subtitulo={
                  m.variacaoMedia != null
                    ? `${fmtPct(m.variacaoMedia)} vs semana anterior`
            ...
            <Grid item xs={6} sm={3}>
              <MetricCard
                titulo="Maior Preço"
                valor={fmt(m.maiorPreco)}
                subtitulo="preço mais alto hoje"
              />
            </Grid>
            <Grid item xs={6} sm={3}>
              <MetricCard
              ...
              <Paper elevation={0} sx={{ p: 2.5, border: "1px solid", borderColor: "grey.200", borderRadius: 3, height: "100%" }}>
                ...
                <ResponsiveContainer width="100%" height={280}>
                  ...
                  >
                    ...
                    />
                    <RechartsTooltip content={<CustomTooltip />} />
                    <ReferenceLine x={m.mediaAtual} stroke="#E02F52" strokeDasharray="4 2" label={{ value: "Média", position: "top", fontSize: 10, fill: "#E02F52" }} />
                    <Bar dataKey="valorDeclarado" name="Preço declarado" radius={[0, 4, 4, 0]}>
                      {dados.top10.map((entry, index) => (
                        <Cell
                          key={index}
                          fill={entry.valorDeclarado <= m.mediaAtual ? "#4CAF50" : "#294D80"}
                        />
                      ))}
                    </Bar>
                  </BarChart>
                  ...
                  {dados.top10.map((p, i) => (
                    <tr key={i} style={{ borderBottom: "1px solid #f5f5f5" }}>
                      ...
                      <td style={{ padding: "8px 6px" }}>
                        {p.bandeira ? (
                          <Chip label={p.bandeira} size="small" variant="outlined" color="secondary" />
                        ) : "—"}
                      </td>
